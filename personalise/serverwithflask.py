...
with open('ratings.dat', 'r') as content_file:
...
def query():
    ...
    with conn.cursor() as cur:
        query = "SELECT movie_id, rating FROM recommendationDB.user_ratings WHERE user_id=%s"
        print query
        ...
        text_file = open("test/" + hash + ".dat", "w+")
        text_file.write(content + processedRow)
        text_file.close()
    ...
    if cache != None:
        ...
        svd = SVD()
        svd.load_data(filename='./test/' + hash + '.dat',
        sep=separator,
